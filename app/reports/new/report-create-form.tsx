...
export function ReportCreateForm({ administrator }: { administrator: AuthenticatedAdministrator }) {
  ...
  function updateField(name: keyof FieldState, value: string) {
    setFields((current) => ({
      ...current,
      [name]: value,
    }));
      ...
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
        <header className="flex flex-col gap-4 rounded-4xl border border-white/60 bg-white/80 p-6 shadow-[0_20px_60px_rgba(76,47,33,0.10)] backdrop-blur lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-2">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-(--ink-muted)">Create Daily Work Report</p>
            <h1 className="text-3xl font-semibold tracking-tight">日報を新規登録</h1>
            <p className="text-sm text-(--ink-soft)">
              登録者: {administrator.name} / {administrator.email}
            </p>
          </div>
          ...
          <form className="grid gap-5 md:grid-cols-2" onSubmit={handleSubmit}>
            ...
            <label className="flex flex-col gap-2 text-sm text-(--ink-soft)">
              ...
              <input
                ...
                onChange={(event) => updateField("clientCode", event.target.value)}
                required
                className="h-12 rounded-2xl border border-black/10 bg-white px-4 outline-none transition focus:border-(--accent-strong)"
                placeholder="C001"
              />
            ...
            <label className="flex flex-col gap-2 text-sm text-(--ink-soft)">
              ...
              <input
                ...
                onChange={(event) => updateField("workCode", event.target.value)}
                required
                className="h-12 rounded-2xl border border-black/10 bg-white px-4 outline-none transition focus:border-(--accent-strong)"
                placeholder="W001"
              />
            ...
            <label className="flex flex-col gap-2 text-sm text-(--ink-soft)">
              ...
              <input
                ...
                onChange={(event) => updateField("laborMinutes", event.target.value)}
                required
                className="h-12 rounded-2xl border border-black/10 bg-white px-4 outline-none transition focus:border-(--accent-strong)"
              />
            </label>
            ...
            <label className="flex flex-col gap-2 text-sm text-(--ink-soft)">
              ...
              <input
                ...
                onChange={(event) => updateField("travelMinutes", event.target.value)}
                required
                className="h-12 rounded-2xl border border-black/10 bg-white px-4 outline-none transition focus:border-(--accent-strong)"
              />
            </label>
            ...
            <label className="flex flex-col gap-2 text-sm text-(--ink-soft)">
              ...
              <input
                ...
                onChange={(event) => updateField("salesAmount", event.target.value)}
                required
                className="h-12 rounded-2xl border border-black/10 bg-white px-4 outline-none transition focus:border-(--accent-strong)"
              />
            </label>
            ...
            <label className="flex flex-col gap-2 text-sm text-(--ink-soft)">
              ...
              <input
                ...
                onChange={(event) => updateField("standardMinutes", event.target.value)}
                className="h-12 rounded-2xl border border-black/10 bg-white px-4 outline-none transition focus:border-(--accent-strong)"
              />
            </label>
