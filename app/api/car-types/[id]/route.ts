...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const item = await findReportMasterById("carType", id);

  if (!item) {
    return apiError({ code: "NOT_FOUND", message: "対象の車種が見つかりません。" }, { status: 404 });
  }
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  const { id } = await context.params;
  const existingItem = await findReportMasterById("carType", id);
  ...
  if (validatedInput.data.name && validatedInput.data.name !== existingItem.name) {
    const duplicateItem = await findReportMasterByName("carType", validatedInput.data.name);

    if (duplicateItem) {
      return apiError({ code: "DUPLICATE_NAME", message: "同じ車種名が既に存在します。" }, { status: 409 });
    }
  }

  return apiSuccess({ item: serializeReportMaster(await updateReportMaster("carType", id, validatedInput.data)) }, { status: 200 });
}
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  const usageCount = await countReportMasterUsage("carType", existingItem.name);

  if (usageCount > 0) {
    return apiError({ code: "MASTER_IN_USE", message: `この車種は ${usageCount} 件の日報で使用中のため削除できません。` }, { status: 409 });
  }

  await deleteReportMaster("carType", id);
