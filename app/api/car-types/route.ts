...
export async function GET() {
  ...
  return apiSuccess({ items: await listReportMasters("carType") }, { status: 200 });
}
...
export async function POST(request: Request) {
  ...
  const existingItem = await findReportMasterByName("carType", validatedInput.data.name);

  if (existingItem) {
    return apiError({ code: "DUPLICATE_NAME", message: "同じ車種名が既に存在します。" }, { status: 409 });
  }

  return apiSuccess({ item: await createReportMaster("carType", validatedInput.data) }, { status: 201 });
}