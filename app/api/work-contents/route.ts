...
export async function GET() {
  ...
  return apiSuccess({ items: await listReportMasters("workContent") }, { status: 200 });
}
...
export async function POST(request: Request) {
  ...
  const existingItem = await findReportMasterByName("workContent", validatedInput.data.name);

  if (existingItem) {
    return apiError({ code: "DUPLICATE_NAME", message: "同じ作業内容名が既に存在します。" }, { status: 409 });
  }

  return apiSuccess({ item: await createReportMaster("workContent", validatedInput.data) }, { status: 201 });
}