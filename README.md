...
- 得意先マスタの新規登録、一覧表示、更新、削除
- 車種、作業場所、作業内容マスタの新規登録、一覧表示、更新、削除
- 日報の請求処理状態の管理と一覧からの更新
- 日報一覧の集計サマリー表示
- 日報一覧の検索条件 URL クエリ同期
- 条件付き CSV 出力
- 選択日報ベースの伝票 PDF 出力
...
- 得意先一覧: [app/clients/page.tsx](app/clients/page.tsx)
- 得意先登録: [app/clients/new/page.tsx](app/clients/new/page.tsx)
- 得意先編集: [app/clients/[id]/edit/page.tsx](app/clients/[id]/edit/page.tsx)
- 車種一覧: [app/car-types/page.tsx](app/car-types/page.tsx)
- 車種登録: [app/car-types/new/page.tsx](app/car-types/new/page.tsx)
- 車種編集: [app/car-types/[id]/edit/page.tsx](app/car-types/[id]/edit/page.tsx)
- 作業場所一覧: [app/work-locations/page.tsx](app/work-locations/page.tsx)
- 作業場所登録: [app/work-locations/new/page.tsx](app/work-locations/new/page.tsx)
- 作業場所編集: [app/work-locations/[id]/edit/page.tsx](app/work-locations/[id]/edit/page.tsx)
- 作業内容一覧: [app/work-contents/page.tsx](app/work-contents/page.tsx)
...
- 車種 API
	- [app/api/car-types/route.ts](app/api/car-types/route.ts)
	- [app/api/car-types/[id]/route.ts](app/api/car-types/[id]/route.ts)
- 作業場所 API
	- [app/api/work-locations/route.ts](app/api/work-locations/route.ts)
	- [app/api/work-locations/[id]/route.ts](app/api/work-locations/[id]/route.ts)
- 作業内容 API
	- [app/api/work-contents/route.ts](app/api/work-contents/route.ts)
	- [app/api/work-contents/[id]/route.ts](app/api/work-contents/[id]/route.ts)
- 日報 API
...
- メール送信設定がある場合、未ログイン状態からパスワード再設定メールを送信できること
...
- 得意先情報を編集できること
- 不要な得意先を削除できること
- 日報登録時に得意先名をプルダウンで選択できること
- 選択した得意先に対応する得意先コードを自動表示できること
...
| 得意先 | string | 必須 | 得意先名 |
| 作業場所 | string | 必須 | 作業実施場所 |
| 担当者(サイン) | string | 必須 | 帳票上の担当者表示 |
| 登録番号または車体番号 | string | 必須 | 対象車両の識別情報 |
| 作業分 | number | 必須 | 作業時間分 |
| 工数分 | number | 必須 | 工数時間分 |
| 移動分 | number | 必須 | 移動時間分 |
| 車種 | string | 任意 | 対象車種 |
...
- 得意先一覧をテーブル形式で表示
- 得意先コード、名称、連絡先、担当者、更新日時を確認できること
- 一覧から得意先の編集、削除を実行できること
- 新規登録画面と編集画面は日報画面と同系統の入力 UI を採用すること

#### 参照マスタ管理画面

- 車種、作業場所、作業内容をそれぞれ一覧形式で表示すること
- 一覧から編集、削除を実行できること
- 日報で使用中の項目は削除不可とすること
...
| name | varchar | 得意先名称 |
| address | varchar | 住所 |
| contact_tel | varchar | 連絡先 TEL |
| contact_email | varchar | 連絡先 Email |
| contact_person | varchar | 担当者 |
| remarks | text | 備考 |
| created_at | timestamp | 作成日時 |
| updated_at | timestamp | 更新日時 |
...
| car_type | varchar | 車種 |
| work_location | varchar | 作業場所 |
| signer_name | varchar | 担当者(サイン) |
| vehicle_identifier | varchar | 登録番号または車体番号 |
| work_code | varchar | 作業内容 |
| customer_status | varchar | 新規/既存 |
| billing_status | varchar | 請求処理 |
...
- メールアドレスは管理者間で一意とする
- 得意先コードは得意先マスタ内で一意とする
- 日報の得意先コードと得意先名は登録時に得意先マスタの値で正規化する
- 車種、作業場所、作業内容は登録時に各マスタの値で正規化する
...
- 得意先 API: 一覧取得、詳細取得、作成、更新、削除
- 参照マスタ API: 車種、作業場所、作業内容の一覧取得、詳細取得、作成、更新、削除
...
- 新規/既存は定義済み値のみ許可する
- メールアドレスは形式チェックを行う
- 得意先マスタでは得意先コード、名称、住所、TEL、Email、担当者を必須とする
- 日報の保存時は指定された得意先コードが得意先マスタに存在することを確認する
- 日報の保存時は指定された車種、作業場所、作業内容が各マスタに存在することを確認する
- 請求処理は未/済のみ許可し、一覧のチェックボックスでも更新可能とする
...
- ログイン済み管理者のみ得意先マスタ管理が可能
- ログイン済み管理者のみ車種、作業場所、作業内容マスタ管理が可能
- ログイン済み管理者のみ管理者追加が可能
...
- DB全削除して管理者のみ再作成: `npm run prisma:reset:admin-only`
...
Client モデルは得意先マスタ管理と、日報入力時の得意先選択の基準データを担う。

主要フィールド:

- code: 得意先コード。ユニーク制約を付与
- name: 得意先名称
- address: 住所
- contactTel: 連絡先電話番号
...
- code に一意制約を設定し、同一コードの重複作成を防止する
- 一覧表示と検索拡張を見越して name に索引を付与する
- 日報テーブルとは外部キーで直接結合せず、登録時に得意先コードと得意先名をスナップショットとして保存する
...
- 初期得意先データは [prisma/seed-data/clients.mjs](prisma/seed-data/clients.mjs) を基準に投入する
- 初期参照マスタデータは [prisma/seed-data/report-masters.mjs](prisma/seed-data/report-masters.mjs) を基準に投入する
- INITIAL_ADMIN_NAME、INITIAL_ADMIN_EMAIL、INITIAL_ADMIN_PASSWORD を用いて初期管理者を投入する
- 管理者のみの初期化は [prisma/reset-admin-only.mjs](prisma/reset-admin-only.mjs) で実行し、全テーブルのデータを消去したうえで初期管理者 1 件だけを再作成する
- 初期得意先データは upsert で投入し、既存コードがある場合は名称や連絡先を同期する
- 初期参照マスタデータも upsert で投入し、日報フォームのプルダウン候補として利用する
...
- clientCode は得意先マスタに存在すること
- clientName は得意先マスタから自動補完し、手入力での不整合を残さないこと
...
- 日報保存時は得意先コードをもとに得意先マスタを参照し、得意先名を正規化して保存する
- 日報で参照中の得意先は削除不可とし、得意先マスタの整合性を保つ
